...
**Step 0 — Weather spike (throwaway, ~1–2 sessions).** Before building anything real: hardcode one route, compute crude distance-based arrival times, call Open-Meteo for a handful of sampled points, and judge whether the output is trustworthy. This attacks the riskiest assumption in the whole plan — arrival-time estimation + a live external weather API — for the price of a day. Delete it afterward; it is not production code.
...
2. **`Trip` + weather + speed engine** — the differentiator, now de-risked. `Trip` (route + departure + stops + optional return) needs the geometry speed engine for arrival times; weather hangs off those times. Introduce Redis + Resilience4j here; slot in OpenTopoData elevation (it feeds the gradient term).
...
- **Elevation backfill (OpenTopoData) is load-bearing**, not cosmetic — it supplies the gradient term. Routes without elevation get a flat-road assumption until backfilled.
- **Weather is per-waypoint at arrival time, sampled** (~every 15–30 min of travel; Open-Meteo is hourly-resolution anyway), interpolated to the arrival hour. Handle: departure beyond forecast horizon (degrade gracefully), past departures, rides crossing hour boundaries. **Timezone explicit** (Georgia UTC+4, no DST).
